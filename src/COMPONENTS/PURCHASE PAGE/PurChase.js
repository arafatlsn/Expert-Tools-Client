...
const PurChase = () => {
  ...
  const {
    ...
    try {
      const url = `https://enigmatic-crag-73288.herokuapp.com/tool?id=${id}`;
  ...
  const pressOrder = async () => {
    ...
    const { data } = await axios.post(
      `https://enigmatic-crag-73288.herokuapp.com/order?id=${id}`,
      purchaseObj
    );
  ...
  return (
    <div>
      {alert && <SuccessAlert message={"Orderd an Item"}></SuccessAlert>}
      <div className="grid lg:grid-cols-2 border p-[.5rem] lg:p-[3rem]">
        <div className="flex justify-center">
          <img
            className="w-[220px] lg:w-[538px] h-[220px] lg:h-[538px] object-contain mb-[1rem] lg:mb-0"
            src={img}
            alt=""
          />
        </div>
        <div>
          ...
          <p className="mt-[.4rem]">
            <span className="underline text-[#E73D50]">Description</span> <br />
            <span>{description}</span>
          </p>
          <hr className="my-[1rem]" />
          <p style={{ color: "rgba(48, 56, 65, .9)" }}>
            <span style={{ color: "rgba(60, 196, 114, 1)" }}>In Stock: </span>
            {quantity}
          </p>
            ...
            <div class="relative z-0 w-full group mt-[2rem] mb-[3rem]">
              <input
                onChange={(e) => {
                  ...
                  if (clicked) {
                    if (
                      Number(e.target.value) < minimum_order ||
              ...
              >
                Order Between: {minimum_order} to {quantity}
              </label>

              {getError && (
                <span className="text-red-500 capitalize">
                  You Can order {minimum_order} to {quantity}
                </span>
              )}
            </div>
            <div>
              <button
                onClick={purchaseProd}
      ...
      {showModal && !getError && (
        <PurchaseModal
          ...
          setPurchaseObj={setPurchaseObj}
        ></PurchaseModal>
      )}

      {confirmModal && (
        <ConfirmModal
          clickAction={pressOrder}
          setConfirmModal={setConfirmModal}
        ></ConfirmModal>
      )}
    </div>
